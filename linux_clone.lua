...
if #args == 1 then
    ...
    if (command == "install") then
        ...
        file.close()
        request.close()
        local request = http.get("https://raw.githubusercontent.com/jfkinslow/CC-Linux-Clone/master/data/usr/bin/add-apt-repo.lua")
        local file = fs.open("/usr/bin/add-apt-repo.lua", "w")
        file.write(request.readAll())
        file.close()
        request.close()
    ...
    elseif (command == "update") then
        ...
        print(fs.delete("/usr/bin/add-apt-repo.lua"))
